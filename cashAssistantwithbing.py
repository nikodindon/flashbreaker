...
def run_cash_show_assistant(nom_jeu,num_quest):    #num_quest pour ouvrir onglet avec la recherche de la question ouverte pour la dernière question de QUIDOL
	...
	def is_negative():
		...
		for i in list_neg:
			if question.lower().find('+'+i+'+') != -1:
				...
				add_log("NEGATION DETECTEE AVEC '"+i+"' - A CONFIRMER",1)
				return True;
	...
	if is_which():   #Question du type "Lequel"
		...
		for ind in list_rep:
			...
			responseFromQuestion2 = get("https://www.bing.com/search?q=" + quest_light + " " + answer[ind] )
			...
			html2 = responseFromQuestion2.text.lower()
			html2 = clean_html(html2)
			
			count.append(hash_count(html, answer[ind]))   #Si réponse exacte trouvée, on la met en avant !
			count2.append(hash_count(html2, answer[ind]))
			
			
			if count[ind] > 2000 or count2[ind] > 2000:                         #Si un gros score lors du hash_count, peut être un problème de recherche (réponse composée de plusieurs mots)
				count[ind] = 0
				count2[ind] = 0
			rep_exact.append(html.count(answer[ind]))
			rep_exact2.append(html2.count(answer[ind]))
			count[ind] = count[ind] + rep_exact[ind]
			count2[ind] = count2[ind] + rep_exact2[ind]
			total = total + count[ind] + count2[ind]  #comptage total 
		...
		responseFromQuestion2 = get("https://www.bing.com/search?q=" + question )
		h = HTMLParser()
		html = responseFromQuestion.text.lower()
		html = clean_html(html)
		html2 = responseFromQuestion2.text.lower()
		...
		count.append(int(0))   #enregistrement vide pour ne pas utiliser l'index 0
		count2.append(int(0))
		count.append(hash_count(html, answer[ind]))
		count2.append(hash_count(html2, answer[ind]))
		rep_exact.append(int(0))
		rep_exact2.append(int(0))
		total = 0
		...
		for ind in list_rep:
			...
			total2=str(count[ind] + count2[ind])
			total3=str(rep_exact[ind]+rep_exact2[ind])
			
	#Affichage des scores des différentes réponses
	print("=============================")
	for ind in list_rep:
		if rep_exact[ind] == 0 :
			print("Score pour R"+str(ind)+" : "+total2)
		else:
			print("Score pour R"+str(ind)+" : "+total2+" dont "+total3+" rep EXACTE(S) trouvee(s) caca")
	print("ATTENTION SI QUESTION AVEC NEGATION PRIVILEGIER LE SCORE LE PLUS BAS OU A ZERO")
	print("=============================")
	if f_debug >= 1:
		for ind in list_rep:
			if rep_exact[ind] == 0 or rep_exact2[ind] == 0:
				add_log("Score pour R"+str(ind)+" : "+total2+"\n",1)    #log
			else:
				add_log("Score pour R"+str(ind)+" : "+total2+" dont "+total3+" rep EXACTE(S) trouvee(s)\n",1) #log
